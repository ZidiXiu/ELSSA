...
def generate_data_semi(file_path, result_path, cut_bound, seed, z_dim, tail=False, return_gx=False):
...
#         df = simulation_cox_weibull(gx, batched_x, lambda_=1e-2, nu_=6,cut_bound=cut_bound, seed=seed)
        ...
        if tail:
            df = simulation_cox_weibull_tail(gx, x_pool, lambda_=1e-2, nu_=2,cut_bound=cut_bound, seed=seed)
        else: 
            df = simulation_cox_weibull(gx, x_pool, lambda_=1e-2, nu_=2,cut_bound=cut_bound, seed=seed)

    # reverse the one-hot-encoded variables
    # reformat the one-hot-encoded variables
    cat0 = x_pool[:,3].reshape(n_samples,1)
    cat1 = x_pool[:,5].reshape(n_samples,1)
    cat2 = (0*x_pool[:,6] + 1*x_pool[:,7]).reshape(n_samples,1)
    cat3 = (0*x_pool[:,8] + 1*x_pool[:,9]+ 2*x_pool[:,10] + 3*x_pool[:,11]).reshape(n_samples,1)
    cat4 = (0*x_pool[:,12] + 1*x_pool[:,13]).reshape(n_samples,1)
    x_new = np.concatenate((x_pool.copy()[:,continuous_variables], cat0, cat1, cat2, cat3, cat4), axis=1)
    n_samples, ncov = x_new.shape
    continuous_variables = np.arange(4)
    categorical_variables = np.setdiff1d(np.arange(ncov), continuous_variables)
    df['x'] = x_new
    new_cov = np.array(['AGE', 'HDL', 'TC', 'SYSBP', 'HRX', 'DIAB', 'SEX',
       'RACE', 'CURRSMK'])
    df['covariates'] = new_cov
...
def generate_data_semi_ohe(file_path, result_path, cut_bound, seed, z_dim, tail=False, return_gx=False):
...
#    C = np.repeat(censor_bound,n)
    ...
    TS = T[sidx]
    XS = X[sidx,:]
    right_truncate = T<cut_bound
    EPS = 0
    # define C only for the right truncated samples
    C = np.random.uniform(0+EPS,cut_bound,size=len(T[right_truncate]))
    CS = np.concatenate([C,np.repeat(cut_bound,n-len(T[right_truncate]))])
    event = 1*(TS<CS)
    nonevent = CS<TS
    # observed time
    YS = TS.copy()
    ...
    t = C*(C<T) +  T*(C>=T)
    return({"t": Y, "e":event, "x":X, "T": T,"C":C})
