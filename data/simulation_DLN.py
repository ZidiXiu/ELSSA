...
#         df = simulation_cox_weibull(gx, batched_x, lambda_=1e-2, nu_=6,cut_bound=cut_bound, seed=seed)
    ...
    cat1 = (0*x_pool[:,6] + 1*x_pool[:,7]).reshape(n_samples,1)
    cat2 = (0*x_pool[:,8] + 1*x_pool[:,9]+ 2*x_pool[:,10] + 3*x_pool[:,11]).reshape(n_samples,1)
    cat3 = (0*x_pool[:,12] + 1*x_pool[:,13]).reshape(n_samples,1)
    x_new = np.concatenate((x_pool.copy()[:,continuous_variables], cat0, cat1, cat2, cat3), axis=1)
